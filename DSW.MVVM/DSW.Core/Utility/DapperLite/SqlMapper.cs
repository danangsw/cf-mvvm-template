...
namespace DSW.Core.Utility.DapperLite
{
    public static partial class SqlMapper
    {
        ...
        private static void AddParamsDictionary(IDbCommand cmd, Dictionary<string,object> data)
        {
            if (cmd == null || data == null) return;
            ...
            {
                IDbDataParameter param = cmd.CreateParameter();
                param.ParameterName = "@" + propertyName.Key;
                param.Value = propertyName.Value ?? DBNull.Value;
                cmd.Parameters.Add(param);
            }
        }
        ...
        public static int ExecuteDictionary(this IDbConnection cnn, string sql, Dictionary<string, object> param, IDbTransaction transaction)
        {
            using (IDbCommand cmd = SetupCommand(cnn, transaction, sql, null, null))
            {
                AddParamsDictionary(cmd, param);
                return cmd.ExecuteNonQuery();
            }
        }
        ...
        public static IEnumerable<T> QueryDictionary<T>(this IDbConnection conn, string sql, Dictionary<string,object> param)
        {
            IList<T> list = new List<T>();
